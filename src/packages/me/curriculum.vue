<template>
  <view>
    <ly-curriculumtable
      ...
      @dateDetailsLi="dateDetailsLi">
    </ly-curriculumtable>
  </view>
</template>
<script setup>
import { ref } from 'vue'
import { scheduleList, getWxWeekList } from '@/service/mine'
...
const palette = ref(['13, 115, 249', '255, 165, 30', '76, 206, 146', '125, 126, 250', '47, 189, 220', '242, 86, 95']) // 课程颜色
          ...
          for (let i in result) {
            if (i === dateList.value[date]) {
              result[i].forEach((item, index) => {
                timetables.value[date][item.scheduleTime - 1] =
                  item.courseName === null ? item.classRoomName : item.courseName
              })
            }
          }
        }
